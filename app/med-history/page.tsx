...
import { ArrowLeft, Plus, Camera, Loader2, Trash2, Calendar, X, Check } from 'lucide-react';
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={handlePhotoAdd} disabled={photoLoading} style={{
              ...
              fontSize: 14, fontWeight: 600, cursor: 'pointer', color: 'var(--foreground)', opacity: photoLoading ? 0.7 : 1
            }}>
        ...
        {showAddForm && (
          <div style={{ background: 'white', borderRadius: 16, padding: 24, marginBottom: 24, border: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <textarea value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))}
                  placeholder="e.g. Prescribed by Dr Smith for Type 2 Diabetes" rows={2}
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 16, justifyContent: 'flex-end' }}>
              <button onClick={() => setShowAddForm(false)} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 20px', fontSize: 14, cursor: 'pointer', color: 'var(--muted)' }}>Cancel</button>
        ...
        {entries.length === 0 && !showAddForm && (
          <div style={{ background: 'white', borderRadius: 16, padding: 48, border: '1px solid var(--border)', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 14, marginBottom: 24, maxWidth: 360, margin: '0 auto 24px' }}>
              Start building your personal medication record.
            </p>
        ...
        {active.length > 0 && (
          <div style={{ marginBottom: 24 }}>
            <h2 style={{ fontSize: 15, fontWeight: 700, color: 'var(--muted)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Current medications ({active.length})</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {active.map(entry => <MedCard key={entry.id} entry={entry} onDelete={handleDelete} deletingId={deletingId} isActive />)}
            </div>
          </div>
        )}
        ...
        {past.length > 0 && (
          <div>
            <h2 style={{ fontSize: 15, fontWeight: 700, color: 'var(--muted)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Past medications ({past.length})</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {past.map(entry => <MedCard key={entry.id} entry={entry} onDelete={handleDelete} deletingId={deletingId} isActive={false} />)}
            </div>
          </div>
        )}

        <p style={{ fontSize: 12, color: 'var(--muted)', textAlign: 'center', marginTop: 32, lineHeight: 1.6 }}>
          This is your personal record only. Not a medical record. Always consult your doctor or pharmacist.
        </p>
      </div>
    </main>
  );
